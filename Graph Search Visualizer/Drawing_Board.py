...
class DrawingBoard(QWidget):
    ...
    # def mouseMoveEvent(cls,event):
    #     if cls.canAddNode and cls.canDragNode:
    #         cls.begin = event.pos()
    #         cls.destination = cls.begin
    #         cls.begin = QPoint(cls.begin.x() + 20, cls.begin.y() + 20)
    #
    #         asciiValue = 65 + len(cls.nodesArray)
    #         nodeName = chr(asciiValue)
    #         node = Node(cls.begin, cls.destination, nodeName)
    #
    #         painter = QPainter(cls.pix)
    #
    ...
    def paintAllNodes(self, color):
        ...
        self.update()
    def paintNodeRED(self, nodeName):
        print('Paint Node RED')
        painter = QPainter(self.pix)
    ...
    def paintWeight(self, enterdFirstNodeNameEdge, enterdSecondNodeNameEdge, weight):
        ...
        # cls.paintQueue.append(("drawText", [midpoint, Qt.AlignCenter, str(weight)]))
        path = QPainterPath()
        path.addText(midpoint, QFont("Arial", 12), str(weight))
